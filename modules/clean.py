...
def clean_data():
    ...
    for file in os.listdir(extracted_folder_path):
        if file.endswith(".csv") or file.endswith(".xlsx"):
            ...
            non_empty_numerical_cols = [col for col in numerical_cols if df[col].notnull().any()]
            if not non_empty_numerical_cols:
                ...
                df[non_empty_numerical_cols] = imputer.fit_transform(df[non_empty_numerical_cols])
                print(f"Imputed values from {file}...")
                
                scaler = MinMaxScaler()
                df[non_empty_numerical_cols] = scaler.fit_transform(df[non_empty_numerical_cols])
